...
import { X, MessageSquare, Send, Sparkles, AlertCircle, Bot, User } from "lucide-react";
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full max-w-md bg-deep-navy border-l border-white/10 z-[100] flex flex-col shadow-2xl"
          >
            {/* Header */}
            <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-black/20">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-instrument text-lg text-white leading-none">AI Tutor</h3>
                  <p className="font-inter text-[10px] text-white/50 uppercase tracking-widest mt-1">
                    {getCourseDisplayName(course)} Curriculum Base
                  </p>
                </div>
              </div>
              <button 
              ...
              {isLimitReached ? (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex flex-col items-center justify-center text-center space-y-2">
                  ...
                  <p className="text-sm font-inter text-red-200">
                    You've reached your 5-message session limit. Please close the drawer and take a break to digest the material!
                  </p>
                </div>
              ) : (
